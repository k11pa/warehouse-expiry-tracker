...
with tab1:
    ...
    if not inwork.empty:
        ...
        if search:
            search = search.strip()
            if len(search) == 6:
                filtered = filtered[filtered['Barcode'].astype(str).str.endswith(search)]
            else:
                filtered = filtered[inwork['Name'].astype(str).str.contains(search, case=False, na=False) |
...
with tab2:
    ...
    input_method = st.radio("Способ ввода:", ["Сфоткать штрих-код", "Ввести штрих-код вручную"], horizontal=True)
    ...
    if input_method == "Сфоткать штрих-код":
        ...
        barcode = st.text_input("Введи последние 6 цифр или полный код")
    
    if barcode:
        barcode_search = barcode.strip()
        if not products.empty:
            products['Barcode'] = products['Barcode'].astype(str).str.strip()
            if len(barcode_search) == 6:
                filtered_products = products[products['Barcode'].str.endswith(barcode_search)]
            else:
                filtered_products = products[products['Barcode'] == barcode_search]
            
            if not filtered_products.empty:
                row = filtered_products.iloc[0]
                name = row['Name']
                ...
                if '2' in status:
                    ...
                    if st.button("✅ Добавить/Обновить", type="primary"):
                        if not expiration.strip():
                            ...
                            new_row = pd.DataFrame({'Barcode': [row['Barcode']], 'Name': [name], 'Expiration': [expiration]})
